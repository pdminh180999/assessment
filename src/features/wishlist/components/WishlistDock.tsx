...
import { ChevronLeft, ChevronRight, Minimize, Plus, Search, Star } from 'lucide-react'
import React, { useEffect, useState } from 'react'
import { getThemeClasses } from '@/shared/constants'
import { WishlistButton } from '@/features/wishlist/components/WishlistButton.tsx'


interface WishlistDockProps {
  activeStackId: string
  stacks: Wishlist[]
  theme: 'light' | 'dark'
  onThemeToggle: () => void
  onStackClick: (stack: Wishlist) => void
  onStackEdit: (stack: Wishlist) => void
  onStackDelete: (id: string) => void
  onAddClick: () => void
  onSearchClick: () => void
  onClose: () => void
  showSearch: boolean
...
export const WishlistDock: React.FC<WishlistDockProps> = ({
                                                            activeStackId,
                                                            stacks,
                                                            theme,
                                                            onThemeToggle,
  ...
  useEffect(() => {
    const currentStack = stacks.find((stack) => stack.id === activeStackId)
    if (currentStack?.items.length === 0) {
      onStackClick(currentStack);
    }
  }, [stacks])
